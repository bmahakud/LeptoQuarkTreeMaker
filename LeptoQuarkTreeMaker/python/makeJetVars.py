...
def makeJetVars(process, sequence, JetTag, suff,  skipGoodJets, storeProperties, SkipTag=cms.VInputTag(), onlyGoodJets=False):
    ...
    if skipGoodJets:
        ...
        from LeptoQuarkTreeMaker.Utils.goodjetsproducer_cfi import GoodJetsProducer
        ...
        if storeProperties>0:
            process.LQTreeMaker.VarsBool.extend(['GoodJets'+suff+':JetID(JetID'+suff+')'])
            process.LQTreeMaker.VectorRecoCand.extend(['GoodJets'+suff+'(Jets'+suff+')'])
        if onlyGoodJets:
            return process
    ...
    ## ----------------------------------------------------------------------------------------------
    '''
    from LeptoQuarkTreeMaker.Utils.subJetSelection_cfi import SubJetSelection
    ...
    ## NJets
    ## ----------------------------------------------------------------------------------------------
    from LeptoQuarkTreeMaker.Utils.njetint_cfi import njetint
    NJets = njetint.clone(
        JetTag = HTJetsTag,
    )
    ...
    ## BTags
    ## ----------------------------------------------------------------------------------------------
    from LeptoQuarkTreeMaker.Utils.btagint_cfi import btagint
    BTags = btagint.clone(
        JetTag       = HTJetsTag,
    ...
    ## MHT
    ## ----------------------------------------------------------------------------------------------
    from LepQuarkTreeMaker.Utils.subJetSelection_cfi import SubJetSelection
